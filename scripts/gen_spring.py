...
LEVEL0_TOPICS = [
    ...
    ("sp-055", "Spring 中的设计模式有哪些？", "设计模式,核心概念", 3),
]

def make_answer(title):
    """生成答案模板"""
    return """## 一、核心概念
...
def generate_questions():
    ...
    for topic_id, title, tags, l1_count in LEVEL0_TOPICS:
        ...
        for l1_idx in range(1, l1_count + 1):
            l1_id = topic_id + "-" + str(l1_idx)
            l1_title = title + "的扩展问题" + str(l1_idx)
            ...
            questions.append({
                ...
                "parent_id": topic_id,
                "title": l1_title,
                "answer": make_answer(l1_title),
                "tags": tags,
                "sort_order": l1_idx
            })
            
            # Level 2
            for l2_idx in range(1, 3):
                l2_id = l1_id + "-" + str(l2_idx)
                l2_title = l1_title + ("的源码实现" if l2_idx == 1 else "的实战应用")
                ...
                questions.append({
                    ...
                    "parent_id": l1_id,
                    "title": l2_title,
                    "answer": make_answer(l2_title),
                    "tags": tags,
                    "sort_order": l2_idx
                })
                
                # Level 3 (每个第一个 Level 2 都生成 Level 3)
                if l2_idx == 1:
                    l3_id = l2_id + "-1"
                    l3_title = l2_title + "的深度解析"
...
def main():
    ...
    print("\n生成完成！")
    print("Level 0:", level_counts[0], "题")
    print("Level 1:", level_counts[1], "题")
    print("Level 2:", level_counts[2], "题")
    print("Level 3:", level_counts[3], "题")
    print("总计:", len(questions), "题")
